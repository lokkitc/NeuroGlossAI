...
class CourseService:
    ...
    async def _create_standard_levels_for_unit(self, unit_template: CourseUnitTemplate) -> list[CourseLevelTemplate]:
        ...
        levels.append(CourseLevelTemplate(unit_template_id=unit_template.id, order=1, type="lesson", total_steps=5))
        levels.append(CourseLevelTemplate(unit_template_id=unit_template.id, order=2, type="lesson", total_steps=5))
        levels.append(CourseLevelTemplate(unit_template_id=unit_template.id, order=3, type="practice", total_steps=10))
        levels.append(CourseLevelTemplate(unit_template_id=unit_template.id, order=4, type="trophy", total_steps=15))
    ...
    async def get_active_course_view(self, user: User) -> dict[str, Any] | None:
        ...
        progress_res = await self.db.execute(select(UserLevelProgress).where(UserLevelProgress.enrollment_id == enrollment.id))
